...
currentsong = req.get(url="https://api.spotify.com/v1/me/player/currently-playing", headers=hed) #Get your Current Song
playlist=req.get(url="https://api.spotify.com/v1/me/playlists", headers=hed) #Get your Playlists
playlist.content

rawjson= json.loads(currentsong.content) #loads the content from the currentsong in json 
if rawjson['is_playing'] is False: 
    ...
    for playlist in items: #Here we catch the information of the Playlists.
        ...
        if collab is False and displayname != '$yourusername' : #catch Protected Playlist where only read
